...
export function GroupMemberSessionsModal({ groupId, learnerId, learnerName, onClose }: Props) {
  const { token } = useAuth();
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      try {
        const payload = await apiRequest<SessionsPayload>(
          `/api/groups/${groupId}/members/${learnerId}/sessions`,
          { token }
        );
      ...
      <div className="modal-content" onClick={(e) => e.stopPropagation()}>
        <div className="modal-header">
          <div>
            ...
            <p className="modal-subtitle">Toutes les sessions des parcours du groupe</p>
          </div>
        ...
        <div className="modal-body">
          ...
          {data && data.sessions.length === 0 ? (
            <div className="empty-state">
              <Calendar size={48} />
              <p>Aucune session trouvée pour ce membre</p>
            </div>
          ) : null}
